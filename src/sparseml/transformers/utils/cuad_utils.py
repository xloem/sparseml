...
def get_raw_scores(examples, preds):
    ...
    for example in examples:
        ...
        if qas_id == 'LohaCompanyltd_20191209_F-1_EX-10.16_11917878_EX-10.16_Supply Agreement__Agreement Date':
            import pdb; pdb.set_trace()
        if isinstance(example.answers, list):
            gold_answers = [answer["text"] for answer in example.answers if normalize_answer(answer["text"])]
        else:
            # Tuan: Examples created from huggingface datasets
            answers = example.answers
            gold_answers = [text for text in answers['text'] if normalize_answer(text)]
            # if not (isinstance(answer["text"], list) and len(answer["text"]) == 1):
            #     import pdb; pdb.set_trace()
            # if normalize_answer(answer["text"][0]):
            #     gold_answers = [answer["text"][0]]
            # else:
            #     gold_answers = []
