...
def get_raw_scores(examples, preds):
    ...
    for example in examples:
        ...
        if isinstance(example.answers, list):
            ...
            # Tuan: Examples created from huggingface datasets have different format
            answers = example.answers
            gold_answers = [text for text in answers["text"] if normalize_answer(text)]
        if not gold_answers:
...
def compute_predictions_logits(
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        assert len(nbest) == len(
            start_indexes
        ), "nbest length: {}, start_indexes length: {}".format(
...
def load_and_cache_features(
    ...
    if os.path.exists(subset_cached_features_file) and not data_args.overwrite_cache:
        logger.info(
            "Loading features from balanced cached file %s", subset_cached_features_file
        )
        dataset = torch.load(subset_cached_features_file)["dataset"]
        examples, features = None, None
    elif os.path.exists(cached_features_file) and not data_args.overwrite_cache:
        ...
        if evaluate:
            ...
            if training_args.local_rank in [-1, 0]:
                logger.info(
                    ...
                    subset_cached_features_file,
                )
                torch.save({"dataset": dataset}, subset_cached_features_file)
    else:
        logger.info("Creating features from scratch")
